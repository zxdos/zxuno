...
[TIP]
====
Por defecto, MacOS indexa y guarda información extra en archivos de los discos externos. Esto se puede minimizar usando estos comandos (suponiendo que la SD se llama `ZXDOSPLUS`):
...
[TIP]
====
También es posible acceder a la BIOS, teniendo un adaptador y un joystick conectado, pulsando dirección abajo en el momento del arranque. Así se accede a la lista de cores instalados. Elegir la última opción (`Enter Setup`) para acceder.
====
...
Además, cuando termina la reproducción completa de un fichero PZX, el reproductor se para automáticamente. También se parará si se encuentra en el fichero una marca STOP, o si se encuentra una marca STOP IF IN 48K y el ZX-UNO está en modo de 48K (sin paginación de 128K).
...
Pulsando `Espacio+F1` se fuerza un reset del core y la carga del menú de arranque, donde se mostrarán las opciones de inicio existentes en el fichero `config.ini`.
