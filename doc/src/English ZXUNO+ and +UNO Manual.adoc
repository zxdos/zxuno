...
[.text-center]
image:img/plusuno_frontal.jpg[pdfwidth=45%]

[.text-center]
image:img/plusuno_trasera.jpg[pdfwidth=70%]
...
For example, on Windows, to format a card with just one FAT16 partition (if the SD card is 4GB or less in size), shown as disk 6 when running `list disk` from `diskpart`:
...
To create two 4GB FAT16 partitions (for example, to use with the MSX core) y and the rest of the space as FAT32 (for cards of more than 8GB in size):
...
To create a 4GB FAT32 partition (for example to use with theAmstrad CPC 6128 core ) and then the rest of the space with another one in FAT32 (for cards of more than 4GB):
...
For example, to format a card whin only one FAT16 partition (if de card is 4B or less in size), 
Por ejemplo, para formatear una tarjeta con una única partición FAT16 (si la tarjeta es de 4GB o menos de tamaño), found as `sdc` in the device list:
...
[.text-center]
image:./img/plusuno_switch.jpg[pdfwidth=70%]
...
[NOTE]
====
El software Maxduino incorporando en <<#_miniduino,el miniduino>> no tiene, por el momento, soporte para los archivos de cinta de Commodore 16, pero posible utilizar <<#_audio_file_conversion,Comodoro>> para generar ficheros `TZX` compatibles
====

[TIP]
====
Es posible utilizar programas como <<#_audio_file_conversion,Audiotap o Comodoro>> con los ficheros `TAP` de cinta de Commodore.
====
...
[NOTE]
====
El software Maxduino incorporando en <<#_miniduino,el miniduino>> no tiene, por el momento, soporte para los archivos de cinta de Commodore 64, pero posible utilizar <<#_audio_file_conversion,Comodoro>> para generar ficheros `TZX` compatibles
====
...
[NOTE]
====
Maxduino, which is used in <<#_miniduino,miniduino>> does not, at this moment, support for Commodore PET tape files, pero posible utilizar <<#_audio_file_conversion,Comodoro>> para generar ficheros `TZX` compatibles
====
...
[TIP]
====
To save a BASIC program using the audio output, you can use the command `CSAVE "NAME"` and `Enter`. To hear the sound while recording, you must enable previously the audio output using the `F10` key.
====

[WARNING]
====
*Do not* keep enabled the recording sound (`F10` key) while loading from a external audio source. This is because the recording sound creates a feedback whith a high pitch sound which breaks loading from audio.
====
...
http://retrowiki.es/viewtopic.php?f=110&t=200037282[Comodoro 2.0] es un script que permite convertir a formato `TAP` y `TZX` para poder realizar cargas por audio en las Commodore Pet, VIC-20, 16 y 64.
...
- USB-Blaster cable. Using the right pins for ZXUNO+, <<#_jtag_cable_connections,as explained earlier>>
- Flash Image file and and recovery file fo ZXUNO+ (LX25). The same ones as for Raspberry Pi, from https://github.com/zxdos/zxuno/tree/master/modflash[Github repository] and https://www.zxuno.com/forum/viewtopic.php?f=37&t=774[ZX-Uno forum]
