...
- Atari standard joystick such as a Sega Mega Drive (Gensis) game pad.
...
- microSD card 32GB capacity or less.
- PS/2 mouse.
...
- `Esc` or joystick button `2` (if a joystick with two or more fire buttons is connected): ZX Spectrum core ROM selection menu.
- `F2`: Enter BIOS setup.
- `1` to `9`: Load the core in the flash location corresponding to that number. On the latest BIOS versions `9` is used to load the core installed to the temporary slot used by the <<#_bob_fossil_browser,ZX3 plugin>>.
- `R`: Load the ZX Spectrum core ROM in "real" mode; switching off esxDOS, new graphics modes and so on.
- `Caps Lock`, `Cursor Down` or joystick `down` (if connected): Core selection menu.
...
- `F2` Enter BIOS setup.
- `1` to `9`: Load the core in the flash location corresponding to that number.  On the latest BIOS versions `9` is used to load the core installed to the temporary slot used by the <<#_bob_fossil_browser,ZX3 plugin>>.
- `Caps Lock` or `Cursor down` or joystick `down` (if connected): Core selection menu.
- `R`: Load the ZX Spectrum core ROM in "real" mode, switching off esxDOS, new graphics modes and so on.
...
- Joystick behaviour connected to the right port and also the emulated joystick using the numeric keypad (Joy Keypad): Kempston, Sinclair 1, Sinclair 2, Protek, Fuller or simulate the keys `Q`,` A` `O`,` P` `Space` and `M`.
- behaviour of a joystick connected to the left port (Joy DE9): Kempston, Sinclair 1, Sinclair 2, Protek, Fuller or simulate the keys `Q`,` A` `O`,` P` `Space` and `M`.
...
- Turbo mode support at 7MHz, 14MHz and 28MHz.
...
- Video output in RGB 15kHz, VGA and DisplayPort.
...
As noted in the installation section, there are a several "dot" commands that are exclusive to the ZXTRES:
...
- `joyconf`: Configure and test for keyboard and DE9 joysticks.
...
- `playrmov`: Play <<#_making_rdm_radastan_movie_files,radastanian format video files `RDM`)>>. This command doesn't work in 48K mode.
...
The `playrmov` "dot" command plays Radastanian format (`RDM`) video files. To convert your own videos you need `makevideoradas`, a utility that's available at this https://svn.zxuno.com/svn/zxuno/software/modo_radastan/videos_radastanianos/[SVN repository].
...
- `Z` or joystick button `1` main fire button: Fire Button `1` for player 1.
- `M` or joystick button `2` main fire button: Fire button `1` for player 2.
- `X` or joystick button `1` secondary fire button: Fire button `1` for player 1 and player 2.
- `T`: Button '*'.
- `Y`: Button '#'.
...
From the multicart menu use the directional controls to choose a ROM and then press fire button `1` to load. Pressing 'Esc' restarts the core and reloads the ROM selection menu.

<<<

=== Neo-Geo

https://es.wikipedia.org/wiki/Neo-Geo[Neo-Geo] is the name of a 16-bit cartridge-based system for arcade as well as home video game consoles released in 1990 by the Japanese video game company SNK. The Neo-Geo system was originally created as a platform for arcade machines (MVS or Multi Video System), but was later also available as a home game console (AES or Advanced Entertainment System).
...
- `Right Ctrl` or button A on controller 1: Player 1's A button.
- `Right Alt` or the B button on controller 1: Player 1's B button.
- `Right Win` or button C on controller 1: Player 1's button C.
- `Righ Shift` or D button on controller 1: Player 1's D button.
- `Start`: Start button of player 1.
- `W`, `A`, `S`, `D` or a controller connected to the right port: Player 2 Directional Controls.
- `Left Ctrl` or the A button on controller 1: Player 2's A button.
- `Left Alt` or the B button on controller 1: Player 2's B button.
- `Left Win` or button C on controller 1: Player 2's button C.
- `Left Shift` or D button on controller 1: Player's D button 2.
...
- Joystick types: Cursor, Sinclar, ZX81 and ZXpand.
...
Press `F5` or joystick button `2` to show or hide the configuration menu. Use the cursor keys and `Enter` to select menu options.
...
You can download the application for Windows from https://worldofspectrum.net/utilities/#tzxtools[World of Spectrum Classic] and for Mac from https://github.com/kounch/playtzx/releases[this GitHub repository]) or compile the source code as <<#_compile_source_code_macOS_or_linux,explained later>>.
