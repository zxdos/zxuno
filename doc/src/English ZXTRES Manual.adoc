...
|[.small]#Amstrad CPC 464#|[.small]#No#|[.small]#No#|[.small]#No#|[.small]#None#|[.small]#No#||[.small]#Doesn't use the SD#
|[.small]#Amstrad CPC 6128#|[.small]#Yes#|[.small]#Yes#|[.small]#No#|[.small]#Any#|[.small]#No#|[.small]#ROMs, Disk Images (`DSK`)#|
|[.small]#Apple IIe#|[.small]#Yes#|[.small]#Yes#|[.small]#No#|[.small]#Any#|[.small]#No#|[.small]#Disk Images (`NIB`)#|
|[.small]#Arcade#|[.small]#Yes#|[.small]#Yes#|[.small]#No#|[.small]#Any#|[.small]#No#|[.small]#Only ROMs (`ARC` and `ROM`)#|
...
|[.small]#PC XT#|[.small]#Yes#|[.small]#Yes#|[.small]#No#|[.small]#Any#|[.small]#No#|[.small]#ROMs, Hard Disk Images (`VHD`)#|[.small]#Requires BIOS#
|[.small]#SAM Coupé#|[.small]#Yes#|[.small]#Yes#|[.small]#No#|[.small]#Any#|[.small]#No#|[.small]#Disk Images (`MGT`, `DSK` and `SAD`)#|
...
[TIP]
====
BIN files can be renamed with the `.BIT` extension and also loaded to the FPGA using the <<#_bob_fossil_nmi_browser,BIT_plugin>> from esxdos.
====
...
The first options is to obtain the latest version of the files that define the core. If the space used is greater than 1179648 bytes it must be split in chunks accordingly into several files. Each of these pieces must be a file called `COREXXy.ZX3` where `XX` is _always_ a two digit number. The `y` part of the name is ignored so you can use longer and more descriptive names (for example `CORE04_example_part1.ZX3`).
...
[TIP]
====
The command line utility https://github.com/zxtres/tools/blob/main/Bit2Bin.c[Bit2Bin_zx3], available at https://github.com/zxtres/tools/tree/main/bin[the ZXTRES GitHub repositories], can create suitable chunks from a complete BIT or BIN file.
====
...
https://es.wikipedia.org/wiki/Amstrad_CPC_6128[Amstrad CPC 6128] was the successor computer to the CPC 464. As a novelty, apart from having more RAM, it incorporated a disk drive.

The https://github.com/ZXMicroJack/amstrad-cpc[ZXTRES version] has been developed by MicroJack, based on the ZX-Uno version of the Amstrad CPC 464 core, created by McLeod.
...
Also the following ROM files are needed which you you can get http://www.cpcwiki.eu/index.php/FPGAmstrad#How_to_assemble_it[from the official wiki] from another older project or from their https://github.com/renaudhelias/FPGAmstrad/raw/master/OS6128_BASIC1-1_AMSDOS_MAXAM.zip[GitHub repository]:
...
If not already there, <<#_cores,install or run the Amstrad CPC6128 core>> on the ZXTRES.
...
The  https://en.wikipedia.org/wiki/[Tetris] arcade machine, licensed and distributed by Atari, in 1988.

The ZXTRES core has been made by somhic, adapting the https://github.com/MrX-8B/MiSTer-Arcade-AtariTetris[original version for MiSTer].
...
The ZXTRES core has been made by somhic, adapting the https://github.com/jotego/jtcps[original version by Jotego].
...
https://en.wikipedia.org/wiki/CP_System[CP System], CPS-1, or https://en.wikipedia.org/wiki/CP_System#CP_System_Dash[CP System Dash] is an enhanced revision of CPS-1.
...
Double Dragon is a https://en.wikipedia.org/wiki/Double_Dragon_(video_game)[video game released for arcade machines] in 1987, developed by Technos Japan.

The ZXTRES core has been made by somhic, adapting the https://github.com/jotego/jtdd[original version by Jotego].
...
Kicker (or Shao-lin's Road) is a https://en.wikipedia.org/wiki/Shao-lin's_Road[video game released for arcade machines] in 1985, by Konami.

The ZXTRES core has been made by somhic, adapting the https://github.com/jotego/jtkicker[original version by Jotego].
...
https://en.wikipedia.org/wiki/Out_Run[Out Run] is a racing video game originally released for arcade machines in 1986.

The ZXTRES core has been made by somhic, adapting the https://github.com/jotego/jtcores/tree/master/cores/outrun[original version by Jotego].
...
The ZXTRES core has been made by somhic, adapting the https://github.com/jotego/jtpang[original version by Jotego].
...
The https://en.wikipedia.org/wiki/Sega_System_16[Sega System 16B] is the second variant of an arcade board released in 1985 by Sega.

The ZXTRES core has been made by somhic, adapting the https://github.com/jotego/jts16[original version by Jotego].
...
[WARNING]
====
At least up to version 20230922, this core does not work correctly when using the <<#_bob_fossil_nmi_browser,BIT plugin>>, and has to be loaded from SPI flash, either directly, or with the <<#_bob_fossil_nmi_browser,ZX3 plugin>>.
====
...
The https://en.wikipedia.org/wiki/SAM_Coupé[SAM Coupe] was a home computer developed and marketed on 1989 by Miles Gordon Technology, based on the Sinclair ZX Spectrum.

The https://github.com/ZXMicroJack/sam-coupe[ZXTRES core] was developed by MicroJack, based on McLeod's https://www.zonadepruebas.com/viewtopic.php?t=6443[original ZX-Uno core], and has the following features:
...
Some of these can be converted with the utility https://simonowen.com/samdisk/[samdisk] to make them work.
====
...
It supports any +3 compatible ROM, but to use a disk image you need one with ZXMMC support (like Garry Lancaster's https://worldofspectrum.org/zxplus3e/[+3e ROMs]).
...
- `BIN/MBUPD` - <<#_esxdos_commands,Firmware update command "DOT">>.
- `BIN/MBINFO` - <<#_esxdos_commands,Command "DOT">> displaying information about the Middle Board firmware
- `BIN/BPLUGINS/BIT` - Latest version of <<#_bob_fossil_nmi_browser,plugin BIT>> for esxdos
...
[TIP]
====
The <<#_jtag,references>> section has information about using JTAG programmer cables different than USB-Blaster.
====
...
- For Linux, a special udev rules file is also needed. https://github.com/trabucayre/openFPGALoader/blob/master/99-openfpgaloader.rules[Available here] to download.
...
For OpenOCD on macOS (using https://brew.sh[Homebrew]):
...
For openFPGALoader on macOS (using https://brew.sh[Homebrew]):
...
[start=3]
. Connect the USB-Blaster, to the computer, and the other end to the ZXTRES <<#_jtag_cable_connections,as explained before>>. If already connected, disconnect and conect to use the new udev rules.

. Copy the file called `recovery.bit` previously downloaded from https://github.com/zxtres/cores/tree/main/recovery[the main Github repository] to the Raspberry Pi. In this example it's at `/home
...
[start=4]
. Connect the USB-Blaster or jump wires if using GPIO. In this case <<#_jtag_cable_connections,as explained before>> connect the FPGA JTAG lines (`TMS` `TDI` `TDO` `TCK` and `GND`) using the wires to the Raspberry Pi https://es.wikipedia.org/wiki/GPIO[GPIO] pins.
...
[start=5]
. Copy the file called `recovery.bit` previously downloaded from https://github.com/zxtres/cores/tree/main/recovery[the main Github repository] to the Raspberry Pi. In this example it's at `/home/pi/zxtres/unbrick/`
...
- FPGA: https://www.xilinx.com/products/silicon-devices/fpga/artix-7.html#productTable[Xilinx Artix] A35T (ZXTRES), A100T (ZXTRES+) and A200T (ZXTRES++).
- Memory: 2MB https://en.wikipedia.org/wiki/SRAM[SRAM] (all models) and 32MB (ZXTRES) or 64MB (ZXTRES+, ZXTRES++) https://en.wikipedia.org/wiki/SDRAM[SDRAM].
- Storage: 32MB https://en.wikipedia.org/wiki/Serial_Peripheral_Interface [SPI] https://en.wikipedia.org/wiki/Flash_memory[Flash]. https://es.wikipedia.org/wiki/MicroSD [MicroSDHC cards] reader.
- Video: https://en.wikipedia.org/wiki/Digital-to-analog_converter[DAC] 888 RGB (https://en.wikipedia.org/wiki/Color_depth#True_color_(24-bit)[truecolor - 24-bit]). DisplayPort and DE-15 (VGA and RGB) digital output.
- Sound output: https://en.wikipedia.org/wiki/Delta-sigma_modulation[Sigma-Delta] and https://en.wikipedia.org/wiki/I²S[I^2^S]. 3.5mm jack connector.
...
- 1 USB A input: To connect keyboards (on models with <<#_middle_board,middle board>> installed).
...
There are several projects that to modify the EEPROM of cheap USB boards with FTDI chips with JTAG protocol speaking capability, so that they behave like clones of an official https://digilent.com/shop/fpga-boards/programmers/[JTAG programming cable].

The supported chips are https://www.ftdichip.com/old2020/Products/ICs/FT2232H.html[FT2232H] and https://www.ftdichip.com/old2020/Products/ICs/FT232H.htm[FT232H].
...
Download the https://github.com/vanbwodonk/ft2232hl-jtag-clone/tree/main/eeprom_binary[new firmware from Github].
...
For OpenOCD use, download the configuration file from the https://github.com/zxtres/wiki/blob/main/Files/OpenOCD_0.10.0-11/FT2232H_FT232H.cfg[ZXTRES GitHub repository], to the working directory (in the examples, `/home/zxtres/unbrick` or `/Users/zxtres/unbrick` for macOS).
...
The commands to use are the same as explained in the <<#_firmware_recovery,recovery instructions>>, but using references to FT2232H, FT232H instead of `usb-blaster`.
